...
class TestWritableService extends WritableService<
  TestResource,
  TestUpdateParams
> {
...
describe("ReadOnlyService", () => {
  describe("list()", () => {
    it("calls correct endpoint with no params", async () => {
      ...
      const data: TestResource[] = [
        { $key: 1, name: "a", description: "desc" },
      ];
  ...
  describe("get()", () => {
    ...
    it("wraps 404 ApiError as NotFoundError with resource name", async () => {
      ...
      vi.mocked(http.get).mockRejectedValueOnce(
        new ApiError(404, "/widgets/99", "Not found"),
      );
  ...
  describe("key normalization", () => {
    ...
    it("handles items with neither $key nor id gracefully", async () => {
      ...
      const svc = new TestReadOnlyService(http);
      vi.mocked(http.get).mockResolvedValueOnce([
        { name: "Orphan", description: "no key" },
      ]);
...
describe("WritableService", () => {
  ...
  describe("delete()", () => {
    ...
    it("wraps 404 as NotFoundError", async () => {
      const http = mockHttp();
      const svc = new TestWritableService(http);
      vi.mocked(http.del).mockRejectedValueOnce(
        new ApiError(404, "/widgets/99", "Not found"),
      );
