...
describe('NodeService', () => {
	...
	describe('actions', () => {
		it('enableMaintenance dispatches to /node_actions', async () => {
			const http = mockHttp();
			const svc = new NodeService(http);
			vi.mocked(http.post).mockResolvedValueOnce(undefined);
			...
			expect(http.post).toHaveBeenCalledWith('/node_actions', {
				body: { node: 1, action: 'enable_maintenance' },
			});
		});

		it('disableMaintenance dispatches disable_maintenance action', async () => {
			const http = mockHttp();
			const svc = new NodeService(http);
			vi.mocked(http.post).mockResolvedValueOnce(undefined);
			...
			expect(http.post).toHaveBeenCalledWith('/node_actions', {
				body: { node: 1, action: 'disable_maintenance' },
			});
		});
		...
		it('clearPStore dispatches clear_pstore action', async () => {
			const http = mockHttp();
			const svc = new NodeService(http);
			vi.mocked(http.post).mockResolvedValueOnce(undefined);

			await svc.clearPStore(1);

			expect(http.post).toHaveBeenCalledWith('/node_actions', {
				body: { node: 1, action: 'clear_pstore' },
			});
		});
		...
		it('listErrors combines with existing filter', async () => {
			...
			expect(http.get).toHaveBeenCalledWith('/logs', {
				params: {
					fields: 'most',
					sort: '-timestamp',
					filter: "(user eq 'admin') and ((level eq 'error') or (level eq 'critical'))",
				},
			});
		});
	});
