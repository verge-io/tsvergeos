...
describe('VMService', () => {
	...
	describe('getConsoleInfo', () => {
		...
		function mockTokenFetch(http: HttpClient, token = mockToken): void {
			(http as unknown as Record<string, unknown>).fetchFn = vi.fn().mockResolvedValue({
				ok: true,
		...
		it('sends credentials to /api/sys/tokens with ybkey login auth', async () => {
			...
			const fetchFn = (http as unknown as Record<string, unknown>).fetchFn as ReturnType<
				typeof vi.fn
			>;
			expect(fetchFn).toHaveBeenCalledWith('https://verge.example.com/api/sys/tokens', {
				method: 'POST',
				headers: {
		...
		it('throws AuthError on invalid credentials', async () => {
			...
			await expect(svc.getConsoleInfo(42, { username: 'bad', password: 'wrong' })).rejects.toThrow(
				'Login required',
			);
		});
		...
		it('returns bearer auth info with API key without acquiring token', async () => {
			...
			expect(info.websocketUrl).toBe('wss://verge.example.com/api/v4/machine_console/7');
		});
