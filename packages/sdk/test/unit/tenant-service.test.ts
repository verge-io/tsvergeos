...
describe('TenantService', () => {
	...
	describe('connect', () => {
		...
		it('prefers url when available', async () => {
			const http = mockHttp();
			const svc = new TenantService(http);
			vi.mocked(http.get).mockResolvedValueOnce({
				...sampleTenant,
				url: 'https://tenant1.example.com',
				_ui_fqdn_value: 'tenant1.proxy.example.com',
				_ui_ip: '10.0.0.1',
				_running: true,
			});

			const tenantClient = await svc.connect(1, { apiKey: 'tenant-key' });

			expect(http.get).toHaveBeenCalledWith('/tenants/1', {
				params: { fields: connectFields },
			});
			expect(tenantClient).toBeDefined();
			expect(tenantClient.host).toBe('https://tenant1.example.com');
		});

		it('falls back to ui_fqdn when url is empty', async () => {
			const http = mockHttp();
			const svc = new TenantService(http);
			vi.mocked(http.get).mockResolvedValueOnce({
				...sampleTenant,
			...
			expect(tenantClient.host).toBe('https://tenant1.proxy.example.com');
		});

		it('falls back to ui_address IP when url and ui_fqdn are empty', async () => {
			const http = mockHttp();
			const svc = new TenantService(http);
			vi.mocked(http.get).mockResolvedValueOnce({
				...sampleTenant,
				url: '',
				_ui_fqdn_value: undefined,
				_ui_ip: '192.168.10.70',
				_running: true,
			});

			const tenantClient = await svc.connect(1, { apiKey: 'tenant-key' });

			expect(tenantClient.host).toBe('https://192.168.10.70');
		});

		it('throws ValidationError when tenant is not running', async () => {
			const http = mockHttp();
			const svc = new TenantService(http);
			vi.mocked(http.get).mockResolvedValueOnce({
				...sampleTenant,
			...
			await expect(svc.connect(1, { apiKey: 'key' })).rejects.toThrow(/not running/);
		});
		...
		it('throws ValidationError when no address is available', async () => {
			...
			await expect(svc.connect(1, { apiKey: 'key' })).rejects.toThrow(/no reachable address/);
		});

		it('adds https:// prefix to bare IPs', async () => {
			const http = mockHttp();
			const svc = new TenantService(http);
			vi.mocked(http.get).mockResolvedValueOnce({
				...sampleTenant,
			...
			const tenantClient = await svc.connect(1, { apiKey: 'key' });

			expect(tenantClient.host).toBe('https://10.0.0.5');
		});

		it('does not double-prefix URLs that already have a protocol', async () => {
			const http = mockHttp();
			const svc = new TenantService(http);
			vi.mocked(http.get).mockResolvedValueOnce({
				...sampleTenant,
			...
			expect(tenantClient.host).toBe('https://tenant.example.com');
		});

		it('proceeds when _running is undefined (status FK not expanded)', async () => {
			const http = mockHttp();
			const svc = new TenantService(http);
			vi.mocked(http.get).mockResolvedValueOnce({
				...sampleTenant,
