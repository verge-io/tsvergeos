...
describe('UserService', () => {
	...
	describe('enable/disable actions', () => {
		it('enable() dispatches to /user_actions with action enable', async () => {
			const http = mockHttp();
			const svc = new UserService(http);
			...
			expect(http.post).toHaveBeenCalledWith('/user_actions', {
				body: { user: 3, action: 'enable' },
			});
		});

		it('disable() dispatches to /user_actions with action disable', async () => {
			const http = mockHttp();
			const svc = new UserService(http);
			vi.mocked(http.post).mockResolvedValueOnce(undefined);
...
describe('APIKeyService', () => {
	...
	describe('create with token', () => {
		...
		it('skips read-back when readBack: false', async () => {
			...
			const result = await svc.create({ user: 1, name: 'my-key' }, { readBack: false });
	...
	describe('getByUserAndName', () => {
		...
		it('throws NotFoundError when no match', async () => {
			...
			await expect(svc.getByUserAndName(1, 'missing')).rejects.toThrow(/not found/i);
		});
	});
...
describe('PermissionService', () => {
	...
	describe('getByIdentityAndResource', () => {
		...
		it('throws NotFoundError when no match', async () => {
			...
			await expect(svc.getByIdentityAndResource(5, 'vms', 999)).rejects.toThrow(/not found/i);
		});
	});
