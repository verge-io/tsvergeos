...
export function hasCredentials(): boolean {
	...
	return Boolean(
		process.env.VERGEOS_API_KEY || (process.env.VERGEOS_USERNAME && process.env.VERGEOS_PASSWORD),
	);
}
...
function discoverSiteAuth(host: string, index: number, suffix: string): SiteEnv | null {
	...
	const verifySsl = process.env[`VERGEOS_VERIFY_SSL${suffix}`]?.toLowerCase() !== 'false';
