...
describeIf('Tenant context connection integration', () => {
	...
	it('should connect to a tenant and get system info', async () => {
		...
		await delay();
		const tenantClient = await hostClient.tenants.connect(firstTenant.$key, tenantCreds);
	...
	it('should connect to multiple tenants independently', async () => {
		...
		await delay();
		const client1 = await hostClient.tenants.connect(tenants[0].$key, tenantCreds);
		await delay();
		const client2 = await hostClient.tenants.connect(tenants[1].$key, tenantCreds);
