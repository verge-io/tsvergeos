...
describeIf("VM service integration", () => {
  ...
  describe("getConsoleInfo", () => {
    ...
    const hasConsoleCreds = Boolean(
      consoleCreds.username && consoleCreds.password,
    );

    it("returns console info with authenticated websocket URL", async () => {
      if (!hasConsoleCreds) {
        // biome-ignore lint: test skip
        return console.log(
          "Skipped: VERGEOS_USERNAME/VERGEOS_PASSWORD not set",
        );
      }
      // Use the known test VM (key=40) which has an OS and guest agent
      await delay();
      const info: VMConsoleInfo = await client.vms.getConsoleInfo(
        40,
        consoleCreds,
      );
      ...
      expect(info.webUrl).toContain("/#/vm-console/40");
