...
describeIf('Recipes & Catalogs integration', () => {
	...
	it('should list VM recipes', async () => {
		...
		for (const recipe of recipes) {
			...
			// VM recipe keys are 40-char hex strings
			expect(String(recipe.$key)).toMatch(/^[0-9a-f]{40}$/);
		}
	});
	...
	it('should list catalogs', async () => {
		...
		for (const catalog of catalogs) {
			...
			// Catalog keys are 40-char hex strings
			expect(String(catalog.$key)).toMatch(/^[0-9a-f]{40}$/);
		}
	});
