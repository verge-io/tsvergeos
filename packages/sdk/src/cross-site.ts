...
import { SiteError, ValidationError, VergeError } from "./errors.js";
...
export type CrossSiteServices = {
  [K in ServiceKeys<VergeClient>]: VergeClient[K] extends ReadOnlyService<
    infer T
  >
  ...
  private async _fanOut<T extends Resource>(
    ...
    const settled = await Promise.allSettled(
      entries.map(async ([name, client]) => {
        ...
        const service = (client as unknown as Record<string, unknown>)[
          serviceName
        ] as ReadOnlyService<T> | undefined;

        if (!service || typeof service.list !== "function") {
          throw new ValidationError(
            `Service '${serviceName}' is not registered on site '${name}'`,
          );
        }
      ...
      if (result.status === "fulfilled") {
        ...
        const cause =
          result.reason instanceof Error
            ? result.reason
            : new VergeError(String(result.reason));
