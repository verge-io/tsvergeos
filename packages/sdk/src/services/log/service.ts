...
export class LogService extends ReadOnlyService<Log> {
	...
	async listByLevel(level: LogLevel, options?: ListOptions): Promise<Log[]> {
		...
		const filter = options?.filter ? `(${options.filter}) and (${levelFilter})` : levelFilter;
	 ...
	 */
	async listByObjectType(objectType: LogObjectType, options?: ListOptions): Promise<Log[]> {
		const typeFilter = `object_type eq '${objectType}'`;
		const filter = options?.filter ? `(${options.filter}) and (${typeFilter})` : typeFilter;
		return this.list({ ...options, filter });
	}
	...
	async listErrors(options?: ListOptions): Promise<Log[]> {
		...
		const filter = options?.filter ? `(${options.filter}) and (${errorFilter})` : errorFilter;
		return this.list({ ...options, filter });
	}
	...
	async listByUser(username: string, options?: ListOptions): Promise<Log[]> {
		...
		const filter = options?.filter ? `(${options.filter}) and (${userFilter})` : userFilter;
		return this.list({ ...options, filter });
	}
	 ...
	 */
	async listSince(timestampMicros: number, options?: ListOptions): Promise<Log[]> {
		const sinceFilter = `timestamp ge ${timestampMicros}`;
		const filter = options?.filter ? `(${options.filter}) and (${sinceFilter})` : sinceFilter;
		return this.list({ ...options, filter });
	}
	...
	async search(pattern: string, options?: ListOptions): Promise<Log[]> {
		...
		const filter = options?.filter ? `(${options.filter}) and (${searchFilter})` : searchFilter;
		return this.list({ ...options, filter });
	}
}
