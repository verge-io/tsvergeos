...
/** FK-expanded fields returned by the connect() query. @internal */
interface TenantConnectFields {
  _ui_fqdn_value?: string;
  _ui_ip?: string;
  _running?: boolean;
}
...
> {
   ...
   * @throws {@link ValidationError} if the tenant is not running or has no reachable address
   *
   * @example
   * ```typescript
   * const tenantClient = await client.tenants.connect(1, {
  ...
  async connect(
    ...
    // Fetch tenant with FK-expanded fields for address resolution and status
    const fields =
      "most,ui_fqdn#fqdn as _ui_fqdn_value,ui_address#ip as _ui_ip,status#running as _running";
    const tenant = await this.http.get<Tenant & TenantConnectFields>(
      `${this.resource}/${key}`,
      { params: { fields } },
    );
    ...
    // Validate tenant is running
    if (tenant._running !== undefined && !tenant._running) {
      throw new ValidationError(
        `Tenant "${tenant.name}" (${key}) is not running`,
        "status",
      );
    }

    // Resolve host: url → ui_fqdn → ui_address IP
    const host = tenant.url || tenant._ui_fqdn_value || tenant._ui_ip;

    if (!host) {
      throw new ValidationError(
        `Tenant "${tenant.name}" (${key}) has no reachable address (url, ui_fqdn, and ui_address are all empty)`,
        "host",
      );
    }

    // Ensure the host has a protocol prefix
    const resolvedHost = host.startsWith("http") ? host : `https://${host}`;
