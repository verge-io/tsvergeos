...
export type OnPowerLoss = 'power_on' | 'last_state' | 'leave_off' | (string & {});

/** Guest operating system family hint. */
export type OSFamily = 'linux' | 'windows' | 'freebsd' | 'other' | (string & {});

/** Boot device priority order. */
export type BootOrder = 'cd' | 'cdn' | 'dc' | 'nc' | 'n' | 'c' | 'd' | 'strict' | (string & {});

/** Cloud-init metadata source configuration. */
export type CloudInitDatasource = 'none' | 'config_drive_v2' | 'nocloud' | (string & {});
...
export type VideoType = 'std' | 'cirrus' | 'vmware' | 'qxl' | 'virtio' | 'none' | (string & {});
 ...
 *
 * The VergeOS WebSocket console endpoint requires a session token tied to a
 * specific user identity for audit logging. Provide either:
 *
 * - `username` + `password` — exchanged for a session token via
 *   `POST /api/sys/tokens` (local VergeOS users)
 * - `token` — a pre-existing session token, e.g., from an OIDC login flow
 */
export type ConsoleAuth = ConsoleCredentials | ConsoleToken;
...
export interface VMConsoleInfo {
	...
	/**
	 * Ready-to-use WebSocket URL for connecting to the console, including
	 * the session token for authentication.
	 *
	 * Format: `wss://<host>/api/v4/machine_console/<consoleKey>?token=<sessionToken>`
	 *
	 * Pass this directly to noVNC, SpiceHTML5, or xterm.js — no additional
	...
	/** Session token used to authenticate the WebSocket connection. `null` when unavailable. */
	token: string | null;
