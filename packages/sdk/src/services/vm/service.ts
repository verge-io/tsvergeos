...
export class VMService extends BaseService<VM, VMCreateParams, VMUpdateParams> {
  ...
  async getConsoleInfo(
    ...
    const isPasswordProtected =
      (result.console_pass_enabled as boolean | undefined) ?? false;
  ...
  private async acquireSessionToken(
    ...
    if (!response.ok) {
      ...
      throw new AuthError(
        text || `Console authentication failed (${response.status})`,
      );
    }
