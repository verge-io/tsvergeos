...
export class NodeService extends WritableService<Node, NodeUpdateParams> {
	...
	async enableMaintenance(key: FlexKey): Promise<void> {
		await this.dispatchAction('enable_maintenance', key);
	}
	...
	async disableMaintenance(key: FlexKey): Promise<void> {
		await this.dispatchAction('disable_maintenance', key);
	}
	...
	/**
	 * Clear persistent storage (pstore) on a node.
	 *
	 * @param key - The node ID
	 */
	async clearPStore(key: FlexKey): Promise<void> {
		await this.dispatchAction('clear_pstore', key);
	}
}
